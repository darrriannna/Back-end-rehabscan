...
export default function ServiceProductPage() {
    ...
    return (
        <>
            <SEO
                title={`${displayedService.title} – Magnetkameraundersökning | RehabScan`}
                description={
                    displayedService.metaDescription ||
                    `${displayedService.title} hos RehabScan. Läs om när undersökningen rekommenderas, vad den kan visa, pris och hur du bokar.`
                }
                canonical={`https://www.rehabscan.se/magnetrontgen/${displayedService.slug}`}
            />
